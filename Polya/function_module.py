...
def substitute(preterm, uvar, coeff, iind):
    return reduce_term(preterm, Environment({uvar.index:(coeff,iind)}))

# Replaces all UVars in preterm that are assigned by env with their designated values.
# Returns a new Term, and a flag True/False representing whether all
# UVars have been replaced.
def reduce_term(preterm,env):
    #print '   REDUCING'
    ...
    if isinstance(preterm, IVar):
        ...
        s, flag = reduce_term(preterm.addpairs[0].term, env)
        t = preterm.addpairs[0].coeff * s
        for ap in preterm.addpairs[1:]:
            s, flag2 = reduce_term(ap.term,env)
            flag = flag and flag2
            t += ap.coeff * s
        return (t, flag)

    elif isinstance(preterm, Mul_term):
        s, flag = reduce_term(preterm.mulpairs[0].term,env)
        t = s**preterm.mulpairs[0].exp
        for mp in preterm.mulpairs[1:]:
            s, flag2 = reduce_term(mp.term,env)
            flag = flag and flag2
            t *= s**mp.exp
        return (t, flag)
...
#             raise No_Term_Exception
        ...
        npairs = [(lambda x,y:(x[0]*y,x[1]))(find_problem_term(H, p.term),p.coeff) for p in u.mulpairs]
        t = npairs[0][0]*IVar(npairs[0][1])
        for p in npairs[1:]:
            t+=p[0]*IVar(p[1])
...
def instantiate(axiom,H):
    ...
    envs = unify(H,axiom.triggers, list(axiom.vars), list(axiom.trig_arg_vars))
    #print 'envs:',envs
    ...
    for env in envs:
        ...
        for c in axiom.clauses:
            ...
            try:
                ...
                if c.lterm == zero:
                    lterm = (0,H.num_terms+1)
                else:
                    red = reduce_term(c.lterm,env)[0]
